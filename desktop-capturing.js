...
chrome.runtime.onMessage.addListener(function(message){
	if (message.handler=='get') {
       ...
       // alert(s);
        ...
        enableSpeakers: 'false' // TRUE
        });
        captureDesktop();
        return;
	}
    else if (message.handler=="voice") {
        s=message.idc;
        chrome.storage.sync.set({
            enableTabCaptureAPI: 'false',
            enableMicrophone: 'true', // TRUE
...
function setupRTCMultiConnection(stream) {
	...
	/*chrome.tabs.query({currentWindow: true, active: true}, function (tab) {
			var tabb = tab[0];		
			var urll = tabb.url 
			var a = urll.indexOf('/site/');
			var b = urll.indexOf('/tool/');
			connection.channel = connection.sessionid = connection.userid = urll.substring(a+6 ,b);
			//alert(connection.sessionid);
	}); */
	
	
    connection.channel = connection.sessionid = connection.userid = s;
    ...
    function roomOpenCallback() {
        ...
        if(room_url_box === true) {
           // var resultingURL = 'https://webrtcweb.com/screen?s=' + connection.sessionid;
			//var resultingURL = 'http://test.com/index.html?s=' + connection.sessionid;
			
            //var resultingURL = 'http://localhost:8080/portal/site/4ab5a3d7-bae3-4180-9142-c58652cb2d5a/tool/8351c357-3722-4389-ab67-7b711226c011/room?s=' + connection.sessionid;
           ...
           /* chrome.windows.create({
				...
				url: "data:text/html, <title>Check hang</title><button onclick= 'redirect()' >Are You Ready?</button><script>function redirect(){window.location.href = '" + resultingURL + "';}</script>",
                type: 'popup',
                width: popup_width,
                height: popup_height,
			...
			/*chrome.tabs.query({status:'complete', currentWindow:'true'}, (tabs)=>{
			tabs.forEach((tab)=>{
				if(tab.url){
					chrome.tabs.update(tab.id,{url: resultingURL});
				 }
				});
			});*/
			...
			chrome.tabs.query({currentWindow: true, active: true}, function (tab) {
					var tabb = tab[0];
					var ses = tabb.url + '?s=' + connection.sessionid;
					var sess = ses.indexOf('?s=');
					var urll = tabb.url 
					var result = urll.slice(0,sess);
					var a = result  + '?s=' + connection.sessionid;
